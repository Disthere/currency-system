...
**Docker** — это платформа для контейнеризации. Контейнер — это изолированный процесс с собственной файловой системой, сетью, переменными окружения. В отличие от виртуальных машин, контейнеры используют ядро хост-ОС, поэтому легче и быстрее.

**Docker Compose** — инструмент для оркестрации нескольких контейнеров. Вместо запуска каждого контейнера вручную (`docker run`), вы описываете всю инфраструктуру в одном YAML-файле. Compose создаёт общую сеть для всех сервисов, чтобы они могли общаться друг с другом.

**Docker Volumes** — механизм персистентности данных. Контейнеры эфемерны (удалил — данные пропали). Volumes хранятся вне контейнера и переживают его удаление. Два типа: named volumes (имя + путь) и bind mounts (путь на хосте + путь в контейнере).

**Docker Health Checks** — механизм проверки готовности сервиса. Без health check Docker считает контейнер "running" сразу после запуска процесса, но процесс может быть ещё не готов принимать запросы (БД загружается, Kafka инициализируется). Health check выполняет команду внутри контейнера и проверяет exit code.

**Docker Network (bridge)** — виртуальный коммутатор. Все контейнеры в одной сети видят друг друга по DNS-именам (имя сервиса = hostname). Это решает проблему динамических IP — не нужно знать IP контейнера, достаточно знать его имя.
...
**Clean Architecture** — архитектурный паттерн, предложенный Robert C. Martin (Uncle Bob). Основная идея: разделение кода на слои с строгой зависимостью от центра к периферии. Центр — Domain (бизнес-сущности и интерфейсы), внешние слои — Application (use-cases), Infrastructure (реализации), API (точки входа). Зависимости направлены внутрь: внешние слои зависят от внутренних, но НЕ наоборот. Это позволяет тестировать бизнес-логику без БД/HTTP, заменять инфраструктуру (PostgreSQL → MongoDB) без изменения ядра, соблюдать Single Responsibility Principle.

**CQRS (Command Query Responsibility Segregation)** — паттерн, разделяющий операции записи (Commands) и чтения (Queries) на разные модели. Команды изменяют состояние, запросы — только читают. В нашем проекте MediatR реализует CQRS: каждый command/query имеет свой handler. Плюсы: независимое масштабирование чтения/записи, оптимизация моделей под конкретную операцию. Минусы: сложность, дублирование кода. Для простых CRUD-приложений CQRS — overengineering.

**Domain-Driven Design (DDD)** — подход к разработке, где фокус на бизнес-домене. Ключевые концепции: Aggregate Root (главная сущность, через которую идёт доступ к кластеру объектов), Value Object (неизменяемый объект, определяемый значениями свойств, например Money), Repository (абстракция доступа к данным), Domain Events (события, возникающие при изменении состояния). В нашем проекте User — Aggregate Root, FavoriteCurrencies — Value Object (список).

**Record types в C#** — ссылочные типы с семантикой значений. В отличие от class, сравниваются по содержимому (все свойства), а не по ссылке. Immutable (неизменяемы) при использовании positional parameters. Поддерживают `with` выражение — создание копии с изменением отдельных свойств. Идеальны для DTO, контрактов, событий, где важна предсказуемость и потокобезопасность.
...
1. **Shared Kernel** (наш вариант) — один проект, который линкуют все сервисы. Просто, но создаёт coupling.
